...
```{r}
separate = function(column_name){
  new_df = data.frame(bwt = birthweight_df$bwt, x = birthweight_df[[column_name]])
  
  names(new_df)[2] = column_name
}

list_df = map(names(birthweight_df), separate)

list_models = map2(list_df, ~lm(bwt ~ .[[2]], data = .x))
